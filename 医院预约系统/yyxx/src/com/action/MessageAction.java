...
public class MessageAction extends ActionSupport {
	...
	//医生留言列表
	@Action(value = "doctormessagelist", results = {
			@Result(name = "success", location = "/doctor/messagelist.jsp"),
			@Result(name = "error", location = "/doctor/login.jsp") })
	public String doctormessagelist() {
		Map session= ServletActionContext.getContext().getSession();
		...
		page.setCurrentPage(this.getCurrentPage());// 设置当前页数
		page.setEveryPage(10);// 设置每页页数
		Results result = messageService.messagelistDoctor(page,doctor.getDid());
		page = result.getPage();// 获得页数对象
		List<Message> messageList = result.getList();// 获得列表对象
		Map request = (Map) ActionContext.getContext().get("request");
		request.put("messageList", messageList);
	...
	@Action(value = "replyadd", results = {
			@Result(name = "success", type = "redirect", location = "/doctormessagelist"),
			@Result(name = "error", location = "/404.jsp") })
